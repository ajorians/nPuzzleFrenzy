...
void Selector::DrawSelector()
{
   ...
   //Left
   thickLineRGBA(m_pScreen, m_pBoardMetrics->GetXPos(nX) + nExtraX, m_pBoardMetrics->GetYPos(nY) + nExtraY, m_pBoardMetrics->GetXPos(nX) + nExtraX, m_pBoardMetrics->GetYPos(nY) + 2*m_pBoardMetrics->GetPieceSize() + nExtraY, SELECTOR_LINE_WIDTH, r, g, b, a);
   //Bottom
   thickLineRGBA(m_pScreen, m_pBoardMetrics->GetXPos(nX) + nExtraX, m_pBoardMetrics->GetYPos(nY) + 2*m_pBoardMetrics->GetPieceSize() + nExtraY, m_pBoardMetrics->GetXPos(nX) + m_pBoardMetrics->GetPieceSize() + nExtraX, m_pBoardMetrics->GetYPos(nY) + 2*m_pBoardMetrics->GetPieceSize() + nExtraY, SELECTOR_LINE_WIDTH, r, g, b, a);
   //Right
   thickLineRGBA(m_pScreen, m_pBoardMetrics->GetXPos(nX) + m_pBoardMetrics->GetPieceSize() + nExtraX, m_pBoardMetrics->GetYPos(nY) + nExtraY, m_pBoardMetrics->GetXPos(nX) + m_pBoardMetrics->GetPieceSize() + nExtraX, m_pBoardMetrics->GetYPos(nY) + 2*m_pBoardMetrics->GetPieceSize() + nExtraY, SELECTOR_LINE_WIDTH, r, g, b, a);
...
void Selector::Move(Direction eDirection)
{
   ...
   if( eDirection == Up ) {
      ...
      if( m_nCurrentY < (m_pBoardMetrics->GetHeight()-2) ) m_nCurrentY++;
#ifdef MOVEMENT_WRAPS
