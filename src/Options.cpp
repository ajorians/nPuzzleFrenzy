...
void Options::ToggleCurrentOption()
{
	if( m_nOptionsIndex == 0 ) {
		m_pConfig->SetRetroStyle(!m_pConfig->GetRetroStyle());
...
#define RETRO_OPTION_Y		(45)
#define ANIMATE_SELECTOR_Y	(115)
#define ANIMATION_SPEED_Y	(160)
void Options::UpdateDisplay()
{
	...
	char buffer[256] = "Use Retro Style: ";
	...
	strcat(buffer, 
"\n\
With Retro Style the pieces are\n\
simple black & white like much\n\
like how they looked on a TI-83+\n\
version of the game.");

	nSDL_DrawString(m_pScreen, m_pFont, 12, RETRO_OPTION_Y, buffer); 

	if( m_nOptionsIndex == 0 ) {
		if( is_classic ) {
			draw_rectangle(m_pScreen, SDL_MapRGB(m_pScreen->format, 255, 255, 255), 7, RETRO_OPTION_Y-5, 312, 60, 1);
		} else {
			draw_rectangle(m_pScreen, SDL_MapRGB(m_pScreen->format, 255, 0, 0), 7, RETRO_OPTION_Y-5, 312, 60, 1);
		}
	}
