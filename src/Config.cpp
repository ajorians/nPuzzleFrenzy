...
#define RETRO_STYLE_DEFAULT		(false)
...
Config::Config()
: m_RetroStyle(RETRO_STYLE_DEFAULT), m_SelectorMovementAnimate(SELECTOR_ANIMATE_DEFAULT), m_FadeSteps(PIECE_FADE_LIMIT), m_BrightenPerStep(PIECE_BRIGHTEN_STEP_AMOUNT), m_PieceMovePerStep(PIECE_STEP_AMOUNT), m_ShowOpeningMoveLimit(SHOW_OPENING_MOVE_LIMIT_DEFAULT)
{
   ArchiveCreate(&m_Archive);

   OpenArchiveFile(m_Archive, "PuzzleFrenzySettings.tns");
   ...
   for(int i=0; i<nSettings; i++) {
      ...
      READ_BOOL_CONFIG_VAR(RetroStyle);
...
Config::~Config()
{
   ...
   WRITE_BOOL_CONFIG_VAR(RetroStyle);
